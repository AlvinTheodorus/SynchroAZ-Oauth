...
#undef HINTERNET_SMART_PTR
			...
			HttpResponse doRequest2(const tstring& method, const tstring& server, const tstring& path, const std::vector<tstring>& headers, const HttpUrlEncodedBody& body) {
#define HINTERNET_SMART_PTR(e) ::std::unique_ptr<void, ::saz::http::detail::InternetHandleDeleter>(e, ::saz::http::detail::InternetHandleDeleter{})
				static const TCHAR* ACCEPT_TYPE[] = {
					_T("application/json"),
					_T("text/json"),
...
#undef HINTERNET_SMART_PTR
		...
		public:
			...
			HttpResponse get(const tstring& server, const tstring& path, const std::vector<tstring>& headers, const HttpUrlEncodedBody& body) {
				return doRequest2(_T("GET"), server, path, headers, body);
			}
		};
	}
}